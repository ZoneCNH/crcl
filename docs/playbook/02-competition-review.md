...
## 与05-competition-scoring-rubric的集成说明
...
- 不应在05-competition-scoring-rubric记录触发条件或历史得分
...
2. 查本文件（02）
   ...
   → 确认触发类型（全维度重计 / 单维度更新）
       ↓
3. 需要复盘 → 用 05-competition-scoring-rubric.md 执行打分
...
5. 打分异常检查（两人独立打分后）
   → 任意单维度差 ≥15分，或总分差 ≥10分
   → 回到 05-competition-scoring-rubric.md 执行"评分异常处理规则"对账流程
...
| 对账要求 | 差≥15分触发05-competition-scoring-rubric对账流程 | 无需对账 |
...
下次触发：（下次复盘的触发条件或日期，例如：
           "下一次季度财报后全维度重计；或USDC市占单月变化>1.5 ppt提前触发"）
```
