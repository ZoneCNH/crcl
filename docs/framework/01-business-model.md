...
每次季度财报先看五项。Q1 2026 的日期和阈值只保留为历史样本，运行时必须从最新 10-Q / 10-K / Circle IR 刷新。
...
└─ 【分支 4】Other revenue 不足以弥补？（平台化补偿不够）
    ...
    → 关键指标：Other revenue 季度值；CPN TPV 年化增速（运行时刷新最新 CPN TPV；2026-02-20 年化 57 亿仅作历史样本）
...
以下为 2026-05-11 Q1 2026 财报当天直接对比使用的历史预期基准，以 FY2025 全年和 Q4 2025 数据为基础合理外推。后续季度不得复用为当前预期，必须新建当季 preview 并刷新最新 SEC filing。
...
1. **储备收益率 ~3.6%**：Q4 2025 实际值 3.8%，历史利率环境（3M T-Bill ~4.2-4.5%，USDXX 与 T-Bill spread 约 50bp）下继续小幅下行，外推约 3.6%。运行时必须用最新 Treasury / USDXX 数据重算。
