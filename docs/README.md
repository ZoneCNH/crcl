...
0. 先执行本文件的 **LLM 运行时刷新协议**，确认当前日期、最新财报、监管状态和数据源可用性。
...
本仓库的文档主要给大模型执行。任何写有日期、"当前"、"截至"、"预计"、"待更新"的内容，默认只是历史快照或检查提示，不得直接当作运行时事实。

大模型每次使用本框架时，必须先建立本次运行上下文：

```text
本次运行日期：
本次运行目的：日监 / 周报 / 财报复核 / 事件复核 / 估值更新
最新 SEC filing：
最新 Circle IR / pressroom 公告：
最新 Circle Transparency 数据日期：
最新监管状态：
最新 competition_score 数据周：
缺失数据：
```
...
5. 无法联网或无法访问官方源时，输出必须标记为 `missing_info` 或 "未刷新"，不能补写最新结论。
6. 所有结论必须带数据日期，格式为 `YYYY-MM-DD`；若使用历史快照，必须写成"历史快照日期"。
...
以下数据必须在每次 Form 4、13F、SEC 归档后刷新。表内描述是待查项，不代表当前事实：
...
| 机构持仓（13F） | 每季度披露，有 45 天滞后 | SEC EDGAR 13F | 对应季度结束后约 45 天披露，运行时查最新 13F |
...
- 13F 有 45 天延迟；每次运行按当前季度和 SEC 最新披露重新确认，不沿用历史披露窗口。
...
本节不保存静态"当前事实"。大模型每次运行必须按下表刷新当前研究基准，并在输出中标注来源日期。

| 需刷新项 | 首选来源 | 写入位置 |
| -------- | -------- | -------- |
| 最新 Circle 财报 / 10-Q / 10-K / 8-K | SEC EDGAR、Circle IR | `metrics/03-quarterly-earnings.md`、`framework/01-business-model.md` |
| 最新 USDC 流通量、7D/30D 变化、储备报告 | Circle Transparency | `metrics/01-daily-watchlist.md` |
| 最新监管状态 | Congress、OCC、Treasury、FinCEN、OFAC、FDIC | `framework/02-regulation.md`、`risk/01-warning-signals.md` |
| 最新竞争数据 | DefiLlama、CoinGecko、CoinMetrics、Dune、RWA.xyz | `metrics/04-competition-dashboard.md` |
| 最新 Coinbase / 渠道暴露 | Coinbase filings、Circle filings | `framework/03-competition.md` |
| 最新稀释股数和筹码事件 | SEC EDGAR、FINRA、Form 4、13F | `valuation/00-valuation-framework.md` |

历史快照可保留在专题文件或 archive 中，但不得覆盖本节的运行时刷新结果。
