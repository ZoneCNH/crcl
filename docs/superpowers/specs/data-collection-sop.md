...
| P0-05 | Circle Reserve Fund 7-day yield | BlackRock | 查 USDXX 基金页面 |
...
| P0-09 | 监管公告（事件触发） | Congress / OCC / Treasury / SEC / CFTC | 见 §2-G |
| P0-10 | Chain status（事件触发） | status.base.org / status.solana.com | 各链状态页 |
...
| P1-05 | USDC adjusted transfer volume | CoinMetrics / Dune | dune.com；coinmetrics.io |
| P1-06 | Active addresses | Dune / Santiment | dune.com |
| P1-07 | USDC velocity（自算） | Dune | 转账量 ÷ 流通量 |
| P1-08 | Exchange USDC balances | Nansen / TokenTerminal | tokenterminal.com |
| P1-09 | DeFi protocol USDC deposits（Aave / Compound） | DefiLlama | defillama.com |
| P1-10 | Coinbase 平台 USDC 余额 / Base 链增长 | TokenTerminal / Dune | 见 §2-G |
| P1-11 | 竞品市值（PYUSD、FDUSD、USDe、Ondo USDY） | CoinGecko / Ethena / Ondo | coingecko.com |
| P1-12 | Tokenized Treasury AUM（RWA.xyz） | RWA.xyz | rwa.xyz |
...
- 连续 2 周 Net mint < 0（净赎回）：升级为 P0 风险，执行 `01-daily-watchlist.md` 触发阈值流程
...
- 连续 4 周市占率下降：升级为 P1 告警，更新 `04-competition-dashboard.md`
...
### D. BlackRock（Circle Reserve Fund 收益率）

**URL**：搜索"BlackRock USD Institutional Digital Liquidity Fund"或"Circle Reserve Fund"；基金代码 USDXX

**获取项**：Circle Reserve Fund 7-day yield

**获取频率**：P0 每日

**操作步骤**：

1. 搜索 BlackRock 官网或 YCharts，找到 USDXX 基金详情页
2. 记录当日"7-Day SEC Yield"（7 日收益率）
3. 对比 3M T-Bill，判断是否已传导：正常情况 USDXX 收益率应接近 3M T-Bill
4. 禁止沿用旧值：每次记录必须标注获取日期
...
5. 8-K 事件触发：财报日前 3 天每日检查，发现后立即阅读并更新框架相关文件
6. Form 4 事件触发：检查内部人交易方向，大额减持记录至 P2 筹码压力项
...
1. 访问 `dune.com` → 搜索对应仪表盘关键词
2. 记录当周数字（精确到亿级）并标注日期
3. 保存仪表盘截图或书签（每周固定截图，防止仪表盘口径变化）
4. 对比上周数字，计算 7D 变化
...
**与 CoinMetrics 差异处理**：容忍度 ±20%，超过须人工核查口径差异；CoinMetrics 清洗更严格，适合严谨量化
...
6. 访问 `rwa.xyz`：记录 Tokenized Treasuries 总 AUM 和 BUIDL AUM
...
3. 当前已知：OCC 于 2026-02-25 发布 GENIUS Act 拟议规则（`nr-occ-2026-9.html`）
...
2. 当前已知：Treasury / FinCEN / OFAC 于 2026-04-08 发布 BSA/AML 与制裁合规拟议规则
...
| CFTC | `cftc.gov/PressRoom/PressReleases` | 商品 vs 证券界定；执法和解 |
...
**获取频率**：月度（BlackRock 基金定期报告）；每周（Circle Transparency 储备构成）

**操作步骤**：

1. 访问 BlackRock USDXX 基金详情页 → 找到月度报告 → 记录 WAM、WAL
2. 访问 `circle.com/transparency` → 查看储备构成：现金比例、国债比例、回购比例
3. 验证流动性要求：每日流动性 ≥ 10%，每周流动性 ≥ 30%（GENIUS Act OCC 规则要求）
4. WAM 拉长（> 60 天）时标注：压力期赎回风险上升
...
| **周五** | 更新 `04-competition-dashboard.md` 竞争评分 | 参照 `05-scoring-rubric.md` 7 维度 | 20 分钟 |
| **周五** | 执行 `06-validation-matrix.md` 周度校验 | 跨文件交叉验证 | 10 分钟 |
| **周五** | 输出本周周报 | `02-weekly-review.md` 模板 | 15 分钟 |
...
**Q：Dune 仪表盘访问很慢或报错？**
→ 备用方案：① CoinMetrics API（需注册）；② TokenTerminal；③ 各链浏览器（Etherscan / SolanaFM / Arbiscan）直接查。
...
**Q：链上数据 Dune vs CoinMetrics 差异很大？**
→ 容忍度 ±20%；超过时用两者各自口径单独列出，不合并计算；优先用 CoinMetrics 做严谨量化口径。
...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| v1.0 | 2026-05-09 | 初始建立。覆盖 P0/P1/P2 全量数据获取 SOP，含 11 个数据源（A–K）、每周操作日历、数据质量规则、FAQ。来源核验：全部有效（参照 `sources.md` 2026-05-08 核验记录）。 |
